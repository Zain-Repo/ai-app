...
unsigned Windows desktop releases. CI validates pull requests, `master` pushes,
...
- `.github/workflows/release.yml` is called only by successful `master` push CI.
  ...
  already public.
- The release job validates the configured HTTPS desktop URL, builds the
  unsigned NSIS installer, publishes with the scoped Actions token through a
  draft/upload/verify/publish sequence, and preserves artifacts for 14 days.
...
No Windows signing secrets or certificate variables are required.
